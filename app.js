...
function route(action, data, sess) {
  switch(action) {
    ...
    case 'getRepertorios': return rows('Repertorios');  // público
    ...
    case 'salvarOverridesRepertorio': return perm(sess,['master','liderequipe','liderbanda'], () => salvarOverridesRepertorio(data));
    ...
    case 'removerMusicoEscala':return perm(sess,['master'], () => removerMusicoEscala(data));
...
function salvarOverridesRepertorio(data) {
  // Salva tom/bpm/versao específicos do repertório sem alterar a biblioteca
  const updates = {
...
function editarRepertorioPerm(data, sess) {
  ...
  if (reps.ok) {
    const rep = reps.data.find(r => r.Id === data.id);
    if (rep && rep.CriadoPor && rep.CriadoPor !== '' && String(rep.CriadoPor) !== String(sess.musicoId||'')) {
...
function getEscalaById(data) {
  ...
  const meus = aceites.ok
    ? aceites.data
        .filter(a => String(a.EscalaId) === String(data.id))
        .map(a => ({ ...a, MusicoNome: musicoMap[a.MusicoId] || a.MusicoId }))
    : [];
  ...
  return ok({ ...e, aceites: meus, overrides });
}
...
function getMinhasEscalas(sess) {
  ...
  // Escalas via aceites (musico foi escalado diretamente)
  ...
  const result = esc.data
    ...
    .filter(e => {
      // Mostrar se: sem repertório, RepReady='sim', ou RepReady vazio (legado/ainda não configurado)
      if (!e.RepertorioId) return true;
      const rep = repMap[e.RepertorioId];
      if (!rep) return true;
      const ready = (rep.RepReady || '').trim().toLowerCase();
      return ready === 'sim' || ready === '';
    })
    .map(e => {
      const aceite = meusAceites.find(a => String(a.EscalaId) === String(e.Id)) || {};
...
function fixColunas() {
  ...
  const fixes = {
    ...
    'Repertorios': ['Id','Nome','BandaId','MusicasIds','Overrides','RepReady','DataCriacao'],
